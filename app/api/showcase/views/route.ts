...
function normalizeNamespace(ns: string) {
  const s = String(ns || "")
    ...
    .replace(/[^a-z0-9_-]+/g, "_")
    .replace(/^_+|_+$/g, "");
  return s || "cunning3d_showcase_views";
}

const COUNTER_API_BASE = String(process.env.SHOWCASE_VIEWS_COUNTER_API_BASE || "")
  .trim()
  .replace(/\/+$/, "") || "https://api.counterapi.dev";

const NAMESPACE = normalizeNamespace(
  process.env.SHOWCASE_VIEWS_NAMESPACE || "cunning3d_showcase_views"
);
...
async function inc(id: string) {
  const url = `${COUNTER_API_BASE}/v1/${NAMESPACE}/${toCounterKey(id)}/up`;
  const res = await fetch(url, { cache: "no-store" });
  if (!res.ok) return 0;
  const json = (await res.json()) as CounterApiResponse;
  const count = json?.count;
