...
It comes with the `AutoTypeTable` component. Learn more about [Auto Type Table](/docs/nodes/components/auto-type-table).
...
You can use it like [Auto Type Table](/docs/nodes/components/auto-type-table), but with additional rules:

- The value of attributes must be string.
- `path` accepts a path relative to the MDX file itself.
- You also need to add [`TypeTable`](/docs/nodes/components/type-table) to MDX components.
