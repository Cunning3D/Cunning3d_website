---
title: Mixedbread
description: Using Mixedbread with Fumadocs UI.
---

## Setup

1. Integrate [Mixedbread Search](/docs/cda/search/mixedbread).

2. Create a search dialog, update `apiKey` and `storeIdentifier` with your desired values.

   <include meta='title="components/search.tsx"'>./mixedbread.tsx</include>

<include>.shared.mdx</include>

### Tag Filter

Optionally, you can add UI for filtering results by tags. Configure [Tag Filter](/docs/cda/search/mixedbread#tag-filter) on search server and add the following:

<include>.tag-filter.mdx</include>
