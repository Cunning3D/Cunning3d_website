...
Fumadocs generates **page slugs** and **page tree** (sidebar items) from your content directory using [`loader()`](/docs/cda/source-api),
...
To customise its output further, you may use [Loader Plugins](/docs/cda/source-api/plugins).
...
On Fumadocs MDX, you can specify a [`schema`](/docs/platforms/collections#schema-1) option to customise frontmatter schema.
...
Fumadocs UI renders root folders as [Sidebar Tabs](/docs/nodes/layouts/docs#sidebar-tabs), which allows user to switch between them.
...
You can add an [`icon` handler](/docs/cda/source-api#icons) to `loader()`.
