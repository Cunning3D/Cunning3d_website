...
See detailed docs for [`links`](/docs/nodes/layouts/links) and [`nav`](/docs/nodes/layouts/nav) options.

### References

<auto-type-table
  cwd path="content/docs/nodes/props.ts"
...
For page tree from [`loader()`](/docs/cda/source-api), it generates the tree from your file structure, see [available patterns](/docs/page-conventions).
...
You can specify a `banner` to the [Docs Layout](/docs/nodes/layouts/docs) component.
...
<auto-type-table cwd path="content/docs/nodes/props.ts" name="SidebarProps" />
